...
angular.module('frontendApp').controller('ActualizarCuentaCtrl', [
	'$scope',
	'$routeParams',
	'accountServices',
	function($scope, $routeParams, accountServices) {
		...
		$scope.actualizarCuenta = function(isValid) {
			if (isValid) {
				...
				// $scope.account._id = {'$oid':$scope.idCuenta};
				accountServices.update($scope.idCuenta, $scope.account);
				$scope.feedbackMessages.push({
					type: 'success',
					text: 'La cuenta [' + $scope.account.name + '] ha sido actualizada correctamente.'
				});
				$scope.account = null;
			} else
				$scope.feedbackMessages.push({
		...
		function cargarCuenta() {		
			var cuenta = accountServices.get($scope.idCuenta);
